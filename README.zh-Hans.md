...
## 🚀 从没用过 git／GitHub／agent？从这里开始

如果这些对你都是新东西，**先跳过本页其他内容**。把下面这段文字整段复制，贴到
你已在用的任何 AI 助手（Claude、Codex、opencode、ChatGPT…）。它会从零带你走完
整个架设——你只需要把简历放进一个文件夹、然后回答问题。你的 AI 会照这套 kit
随附给自身 agent 的同一份指南走：`docs/getting-started.md`。

> **⬇️ 复制下面方框内的全部内容 ⬇️**

```text
你要从零帮我架设「job-hunt-kit」。我对 GitHub、git、命令行、Python、AI agent
完全没经验——所以请尽量帮我做，或每个步骤都用平易近人、非技术性的中文说明。

项目在此：https://github.com/konggirljin/job-hunt-kit

AI 应该做的是：
1. 先读该 repo 里的 docs/getting-started.md——它教你如何引导完全新手。
2. 告诉我确切要准备哪些文件、以及如何在我的电脑上设置 git repo。我知道至少要
   准备 .docx 档的简历；请说明如果我只有 PDF 该怎么取得 .docx。
3. 一步步带我把这个 GitHub kit 装到我的电脑上。
4. 通过问我问题，帮我填好「profile」文件夹（我的简历、我的诚实笔记、其他
   不在简历上的背景、我的求职偏好）。
5. 设定好我的 AI agent，让它能用 kit 的技能。
6. [选用] 架设完成后，AI 可以问我要不要跑内建测试、让我看它运作正常，并示范
   一个真实任务（例如「改造我的简历」或「找工作」），让我以后知道怎么用。

重要：把我当成什么都不懂、从没用过 git——每个名词都要解释，能帮我做的技术步骤
就帮我做，非必要别叫我打字。先从「告诉我下一步该怎么做」开始。
```

对 git 已经很熟？直接看 [快速开始](#快速开始5-分钟)。
