...
## 完全沒碰過 git／GitHub／agent？讓 AI 幫你裝

你不必懂任何東西。把下面這段提示詞複製貼到你的 AI 助手（Claude、Codex、
opencode、ChatGPT…），它會從零一步步帶你走完——你唯一要親手做的，就是
把你的履歷放進一個資料夾、然後回答問題。你的 AI 會照這套 kit 自己的指南
走：`docs/getting-started.md`。

<details>
<summary>點擊展開可複製的提示詞</summary>

```text
你要從零幫我架設「job-hunt-kit」。我對 GitHub、git、命令列、Python、AI agent
完全沒經驗——所以請盡量幫我做，或每個步驟都用平易近人、非技術性的話說明。

專案在此：<貼上 git clone 網址或 GitHub repo 連結>

我想請你：
1. 先讀該 repo 裡的 docs/getting-started.md——它教你如何引導像我這樣的
   完全新手。
2. 告訴我要準備哪些檔案。我知道至少要準備 .docx 檔的履歷；請說明如果我只有
   PDF 該怎麼轉成 .docx。
3. 一步步帶我把這套 kit 裝到我的電腦上。
4. 幫我填好「profile」資料夾（我的履歷、我的誠實筆記、其他不在履歷上的背景、
   我的求職偏好）。
5. 設定好我的 AI agent，讓它能用 kit 的技能。
6. 接著跑內建的測試讓我看它運作正常，並示範一個真實任務（例如「改造我的履歷」
   或「找工作」），讓我以後知道怎麼用。

重要：把我當成什麼都不懂——每個名詞都要解釋，能幫我做的技術步驟就幫我做，
非必要別叫我打字。先從「告訴我要準備什麼」開始。
```
